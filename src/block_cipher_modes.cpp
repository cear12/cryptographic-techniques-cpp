...
Aes128::Key EncKeyFor(const std::vector<std::uint8_t> &key) {
  ...
  Aes128::Key aes_key{};
  std::copy(digest.begin(), digest.begin() + static_cast<long>(aes_key.size()),
            aes_key.begin());
  return aes_key;
}
...
void XorBlock(std::uint8_t *out, const std::uint8_t *a, const std::uint8_t *b,
              std::size_t n) {
...
std::vector<std::uint8_t> CtrXor(const Aes128 &aes,
  ...
  std::copy(
      ...
      counter_block
          .begin()); // nonce occupies the low bytes; upper bytes start at 0
  ...
  while (offset < data.size()) {
    ...
    for (int i = 0; i < 8; ++i) {
      counter_block[static_cast<std::size_t>(15 - i)] =
    ...
    XorBlock(out.data() + offset, data.data() + offset, keystream.data(),
             chunk);
...
CipherResult
BlockCipherModes::EncryptCbcWithIv(const std::vector<std::uint8_t> &plaintext,
  ...
  std::uint8_t pad_value =
      static_cast<std::uint8_t>(16 - (plaintext.size() % 16));
  ...
  for (std::size_t offset = 0; offset < padded.size(); offset += 16) {
    Aes128::Block block{};
    std::copy(padded.begin() + static_cast<long>(offset),
              padded.begin() + static_cast<long>(offset) + 16, block.begin());
    ...
    std::copy(encrypted.begin(), encrypted.end(),
              result.ciphertext_.begin() + static_cast<long>(offset));
    previous = encrypted;
  }
  return result;
}
...
BlockCipherModes::DecryptCbcWithIv(const CipherResult &in,
                                   const std::vector<std::uint8_t> &key) {
  if (in.ciphertext_.empty() || in.ciphertext_.size() % 16 != 0 ||
      in.iv_or_nonce_.size() != 16) {
  ...
  for (std::size_t offset = 0; offset < in.ciphertext_.size(); offset += 16) {
    Aes128::Block block{};
    std::copy(in.ciphertext_.begin() + static_cast<long>(offset),
              in.ciphertext_.begin() + static_cast<long>(offset) + 16,
              block.begin());
...
CipherResult BlockCipherModes::EncryptCtrWithNonce(
...
std::vector<std::uint8_t>
BlockCipherModes::DecryptCtrWithNonce(const CipherResult &in,
                                      const std::vector<std::uint8_t> &key) {
  Aes128 aes(EncKeyFor(key));
  return CtrXor(aes, in.iv_or_nonce_,
                in.ciphertext_); // CTR decrypt == CTR encrypt
}

CipherResult BlockCipherModes::EncryptGcmStyle(
    const std::vector<std::uint8_t> &plaintext,
    const std::vector<std::uint8_t> &key,
  ...
  mac_input.insert(mac_input.end(), associated_data.begin(),
                   associated_data.end());
  mac_input.insert(mac_input.end(), result.ciphertext_.begin(),
                   result.ciphertext_.end());
  auto tag = HmacSha256(MacKeyFor(key), mac_input);
  result.tag_.assign(tag.begin(), tag.end());
  return result;
}

std::vector<std::uint8_t> BlockCipherModes::DecryptGcmStyle(
    const CipherResult &in, const std::vector<std::uint8_t> &key,
    const std::vector<std::uint8_t> &associated_data) {
  std::vector<std::uint8_t> mac_input = in.iv_or_nonce_;
  mac_input.insert(mac_input.end(), associated_data.begin(),
                   associated_data.end());
  mac_input.insert(mac_input.end(), in.ciphertext_.begin(),
                   in.ciphertext_.end());
  ...
  if (in.tag_.size() != expected_tag.size() ||
    ...
    throw std::runtime_error("decryptGcmStyle: authentication tag mismatch "
                             "(tampered ciphertext or wrong key/AAD)");
  }
