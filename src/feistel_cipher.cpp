...
FeistelCipher::KeySchedule FeistelCipher::GenerateKeySchedule(const std::vector<std::uint8_t>& master_key) {
    ...
    for (int i = 0; i < 4; ++i) words[static_cast<std::size_t>(i)] = BytesToWord(master_key.data() + i * 4);
...
FeistelCipher::Block FeistelCipher::Encrypt(const Block& plaintext, const KeySchedule& key_schedule) {
    std::uint32_t l = BytesToWord(plaintext.data());
    std::uint32_t r = BytesToWord(plaintext.data() + 4);

    for (int i = 0; i < kRounds; ++i) FeistelRound(l, r, key_schedule[static_cast<std::size_t>(i)]);
...
FeistelCipher::Block FeistelCipher::Decrypt(const Block& ciphertext, const KeySchedule& key_schedule) {
    std::uint32_t l = BytesToWord(ciphertext.data());
    std::uint32_t r = BytesToWord(ciphertext.data() + 4);

    for (int i = kRounds - 1; i >= 0; --i) FeistelRound(l, r, key_schedule[static_cast<std::size_t>(i)]);
