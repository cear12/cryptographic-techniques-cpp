...
std::vector<std::uint8_t> Pbkdf2HmacSha256(const std::vector<std::uint8_t>& password,
    ...
    for (std::uint32_t block_index = 1; derived.size() < derived_key_length; ++block_index) {
        ...
        for (std::uint32_t i = 1; i < iterations; ++i) {
            u_digest = HmacSha256(password, std::vector<std::uint8_t>(u_digest.begin(), u_digest.end()));
            for (std::size_t b = 0; b < kHashLen; ++b) t[b] = static_cast<std::uint8_t>(t[b] ^ u_digest[b]);
        }
...
std::vector<std::uint8_t> HkdfSha256(const std::vector<std::uint8_t>& salt, const std::vector<std::uint8_t>& ikm,
    ...
    std::vector<std::uint8_t> effective_salt = salt.empty() ? std::vector<std::uint8_t>(kHashLen, 0x00) : salt;
    ...
    while (okm.size() < output_length) {
        ...
        okm.insert(okm.end(), previous_t.begin(), previous_t.begin() + static_cast<long>(take));
        ++counter;
    }
    return okm;
}
...
std::vector<std::uint8_t> MemoryHardKdf(const std::vector<std::uint8_t>& password,
    ...
    for (std::uint32_t round = 0; round < 64; ++round) {
        ...
        for (int b = 0; b < 4; ++b) index = (index << 8) | mix_state[static_cast<std::size_t>(b)];
        index %= cost_factor;

        std::vector<std::uint8_t> mix_input(mix_state.begin(), mix_state.end());
        mix_input.insert(mix_input.end(), buffer[index].begin(), buffer[index].end());
