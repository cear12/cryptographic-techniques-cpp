...
constexpr std::uint32_t kScryptCostFactor =
    16'384; // 2^14, a common scrypt N default
...
DerivedKeys SplitIntoKeys(const std::vector<std::uint8_t> &material,
                          std::vector<std::uint8_t> salt,
                          std::uint32_t iterations) {
  DerivedKeys keys;
  keys.encryption_key_.assign(material.begin(),
                              material.begin() + static_cast<long>(kKeyLength));
  keys.mac_key_.assign(material.begin() + static_cast<long>(kKeyLength),
                       material.end());
...
DerivedKeys
KeyDerivationTechniques::DeriveKeysPbkdf2(const std::string &password) {
  auto salt = RandomSalt(16);
  auto material = cryptotech::Pbkdf2HmacSha256(ToBytes(password), salt,
                                               kPbkdf2Iterations, kTotalLength);
  return SplitIntoKeys(material, salt, kPbkdf2Iterations);
}

DerivedKeys
KeyDerivationTechniques::DeriveKeysScrypt(const std::string &password) {
  auto salt = RandomSalt(16);
  auto material = cryptotech::MemoryHardKdf(ToBytes(password), salt,
                                            kScryptCostFactor, kTotalLength);
  return SplitIntoKeys(material, salt, kScryptCostFactor);
}

DerivedKeys KeyDerivationTechniques::DeriveKeysHkdf(
  ...
  auto material =
      cryptotech::HkdfSha256({}, shared_secret, context_info, kTotalLength);
