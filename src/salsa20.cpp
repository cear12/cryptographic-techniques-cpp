...
std::array<std::uint8_t, 64> Salsa20Block(const Salsa20::Key &key,
                                          const Salsa20::Nonce &nonce,
                                          std::uint64_t counter) {
  static constexpr std::uint32_t kSigma[4] = {
  ...
  for (int round = 0; round < 10;
       ++round) { // 10 double-rounds = 20 rounds total
    // Column rounds.
  ...
  for (int i = 0; i < 16; ++i) {
    std::uint32_t sum = working[static_cast<std::size_t>(i)] +
...
XorWithKeystream(const std::vector<std::uint8_t> &data, const Salsa20::Key &key,
                 const Salsa20::Nonce &nonce) {
  ...
  while (offset < data.size()) {
    ...
    for (std::size_t i = 0; i < chunk; ++i)
      out[offset + i] = static_cast<std::uint8_t>(data[offset + i] ^ block[i]);
    offset += chunk;
  }
  return out;
}
...
std::vector<std::uint8_t>
Salsa20::Encrypt(const std::vector<std::uint8_t> &plaintext, const Key &key,
                 const Nonce &nonce) {
  return XorWithKeystream(plaintext, key, nonce);
}

std::vector<std::uint8_t>
Salsa20::Decrypt(const std::vector<std::uint8_t> &ciphertext, const Key &key,
                 const Nonce &nonce) {
  return XorWithKeystream(ciphertext, key,
                          nonce); // XOR stream cipher: decrypt == encrypt
}
