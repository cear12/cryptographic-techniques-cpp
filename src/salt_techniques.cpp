...
PasswordHash SaltTechniques::HashPassword(const std::string& password) {
    ...
    result.hash_ = cryptotech::Pbkdf2HmacSha256(ToBytes(password), result.salt_, result.iterations_, kHashLength);
    return result;
}

bool SaltTechniques::VerifyPassword(const std::string& password, const PasswordHash& stored) {
    auto candidate = cryptotech::Pbkdf2HmacSha256(ToBytes(password), stored.salt_, stored.iterations_, stored.hash_.size());
    ...
    if (candidate.size() != stored.hash_.size()) return false;
    std::uint8_t diff = 0;
    for (std::size_t i = 0; i < candidate.size(); ++i) diff |= static_cast<std::uint8_t>(candidate[i] ^ stored.hash_[i]);
    return diff == 0;
}
